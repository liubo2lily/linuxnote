...
{
	/* 功能为分辨是哪一个hwirq，调用对应的irq_desc[].handle_irq */
	...
	generic_handle_irq(irq_find_mapping(virtual_intc_domain, hwirq));	// hwirq=>virq
			...
			irq_hw_number_t hw)
{
	...
	irq_set_chip_and_handler(virq, &virtual_intc_irq_chip, handle_edge_irq); /* handle_edge_irq(edge边沿触发)就是handleC */
...
static int virtual_intc_probe(struct platform_device *pdev)
{
	...
	/* 1.2 设置它的irq_desc[].handle_irq，功能为分辨是哪一个hwirq，调用对应的irq_desc[].handle_irq */
	irq_set_chained_handler_and_data(irq_to_parent, virtual_intc_irq_handler, NULL);

	/* 2. 分配/设置/注册一个irq_domain */ /* legacy linear hierarchy */
	...
	virtual_intc_domain = irq_domain_add_linear(np, 4, &virtual_intc_domain_ops, NULL);
